...
impl MockDaStrategy {
    fn new() -> Self {
        Self { tx_hash: StdMutex::new(None) }
    }
}
...
impl ProofProvider for TestProofProvider {
    async fn get_proof(&self, _batch_id: &BatchId, _public_inputs: &[u8]) -> Result<ProofResponse, DomainError> {
        Ok(ProofResponse {
...
async fn test_batch_lifecycle() {
    ...
    let storage = Arc::new(SqliteStorage::new(db_url).await.expect("Failed to create storage"));
    ...
    storage.save_batch(&batch).await.expect("Failed to save batch");

    // 4. Run one iteration of orchestrator (Discovered -> Proving)
    orchestrator.process_pending_batches().await.expect("Failed to process");
